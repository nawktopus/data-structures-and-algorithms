...
const count = (target, input) => {
  let count = 0;
  input.flat().map(n => n === target ? count++ : false);
  return count;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  input.map(e => e.filter(n => typeof n === 'number' ? n % 5 === 0: false)).map(e => e.map(n => Math.pow(2,n)));
...
let findShortest = (data) => {
  return data.reduce((p, c)=> p.height > c.height ? p : c,0).name;
};
