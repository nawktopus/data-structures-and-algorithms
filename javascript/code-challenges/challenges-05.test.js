...
const addValues = (arr) => {
  ...
  let newAddValues = arr.reduce((p, c) => p + c, initialValue);
  return newAddValues;
};
...
const addPurchases = (arr) => {
  const initialValue = 0;
  let newAddValues = arr.reduce((p, c) => p + c.purchasePrice, initialValue);
  return newAddValues;
};
...
const returnNames = (arr) => {
  const newReturnNames = arr.reduce((a, v, index)=> {
    a.push(v.name);
    return a;
  }, []);
  return newReturnNames;
};
